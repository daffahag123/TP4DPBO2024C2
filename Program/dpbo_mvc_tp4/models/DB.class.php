<?php

// Definisi kelas DB
class DB
{
    // Variabel untuk menyimpan informasi koneksi database
    var $db_host;
    var $db_user;
    var $db_pass;
    var $db_name;
    
    // Variabel untuk menyimpan koneksi database dan hasil query
    var $db_link;
    var $result;

    // Konstruktor untuk inisialisasi informasi koneksi database
    function __construct($db_host, $db_user, $db_pass, $db_name)
    {
        $this->db_host = $db_host;
        $this->db_user = $db_user;
        $this->db_pass = $db_pass;
        $this->db_name = $db_name;
    }

    // Fungsi untuk membuka koneksi ke database
    function open()
    {
        $this->db_link = mysqli_connect($this->db_host, $this->db_user, $this->db_pass, $this->db_name);
    }

    // Fungsi untuk mengeksekusi query
    function execute($query)
    {
        $this->result = mysqli_query($this->db_link, $query);
        return $this->result;
    }

    // Fungsi untuk mendapatkan hasil query
    function getResult()
    {
        return mysqli_fetch_array($this->result);
    }

    // Fungsi untuk menutup koneksi ke database
    function close()
    {
        mysqli_close($this->db_link);
    }
}

?>
